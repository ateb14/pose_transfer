...
def detect(path,pose_path):
    for cnt, name in tqdm(enumerate(os.listdir(path)), total=len(os.listdir(path))):
        if name.split('.')[-1] != 'png':
            continue
        # if name != 'MEN_Sweaters_id_00000289_02_2_side.png':
        #     continue
        # if cv2.imread(os.path.join(pose_path, name)) is not None:
        #     continue
        try:
            np.loadtxt(os.path.join(pose_path, name.split('.')[0]+'.txt')) is not None
        except:
            print(name)
